...
app.post('/monitorar', async (req, res) => {
  ...
  lastSeenContact = [];
  try {
    await socket.onWhatsApp(contactToMonitor);
...
async function startBot() {
  ...
  socket.ev.on('connection.update', (update) => {
    ...
    if (connection === 'open') {
      ...
      qrData = '';
      if (contactToMonitor) {
        socket.onWhatsApp(contactToMonitor)
          .catch(() => null)
          .then(async () => {
            await socket.presenceSubscribe(contactToMonitor);
            console.log(`🔎 Re-subscrito para ${contactToMonitor}`);
          })
  ...
  socket.ev.on('presence.update', (update) => {
    console.log('RAW presence.update →', update);
